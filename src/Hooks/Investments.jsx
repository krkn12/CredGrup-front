  ...
  useEffect(() => {
    const fetchInvestment = async () => {
      try {
        ...
        setInvestmentData(response.data || { amount: 0, initialDate: null, canWithdraw: false, profit: 0 });
      } catch (error) {
        console.error('[Investments] Erro ao buscar investimento:', error);
      }
    };
    if (currentUser) fetchInvestment();
  }, [currentUser]);
  ...
  const handleInvest = async () => {
    ...
    try {
      ...
      const updatedInvestment = {
        ...
        canWithdraw: response.data.investment.canWithdraw || false,
        profit: response.data.investment.profit || (response.data.investment.amount * 0.15),
      };
